...
def fonceurabut(m,goal):
    ...
    b=m.state.ball
    n=0
    if (goal-m.b_p).norm<30:
        s=goal-m.b_p
    else:
        s=goal-m.b_p
        s.scale(1.0/(goal-m.b_p).norm)
...
def passage2v1(mystate):
    ...
    e_p = mystate.state.player(e_t,0).position
    ...
    if d<1:
        ...
        return SoccerAction(calculdepos(b,p)-p,
                        Vector2D(0,0))
                        
                        
def demarque2v1(mystate):
    e_t = 3 - mystate.id_team
    #enemy team (int)
    ...
    a_d_vec = b_p-a_p
    #distance allie (vector)
    butlist=[Vector2D(0,0),Vector2D(0,45),Vector2D(0,90),Vector2D(75,0),Vector2D(75,45),Vector2D(75,90),Vector2D(150,0),Vector2D(150,45),Vector2D(150,90)]
    but=Vector2D(0,0)
    for i in butlist:
        if i.distance(e_p)>but.distance(e_p):
            but=i
            
    return fonceurabut(mystate,but)
